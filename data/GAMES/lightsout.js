class LO {
	constructor (room, user, size) {
		...
		this.size = size || [5, 5];
		this.board = Array.from({ length: size[0] }).map(row => Array.from({ length: size[1] }).map(t => Bot.AFD ? 1 : 0));
		...
		this.click = (a, b) => {
			...
			this.moves.push([a, b]);
			if (!this.board.reduce((a, b) => a + b.reduce((c, d) => c + d, 0),  Bot.AFD ? - 1 * this.size[0] * this.size[1] : 0)) return this.ended = true;
			return false;
		};
